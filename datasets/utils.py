...
def StratifiedShuffleSplit_tr_te(n_splits=5, train_set=0.7, te_set=0.3, random_state=42, n_sub=1009):
    if n_sub == 1009:
        data = np.load('D:/dataset/SEED-IV/SEED-IV_pcp/cpac/filt_noglobal/cc200/NPY/1009_abide.npy',
                       allow_pickle=True).item()
    else:
        data = np.load('D:/dataset/SEED-IV/SEED-IV_pcp/cpac/filt_noglobal/cc200/NPY/1035_abide.npy',
                       allow_pickle=True).item()
...
def StratifiedShuffleSplit_tr_vl_te(n_splits=5, train_set=0.7, val_set=0.1 ,random_state=0, n_sub=1035):
    if n_sub == 1009:
        data = np.load('D:/dataset/SEED-IV/SEED-IV_pcp/cpac/filt_noglobal/cc200/NPY/1009_abide.npy', allow_pickle=True).item()
    else:
        data = np.load('D:/dataset/SEED-IV/SEED-IV_pcp/cpac/filt_noglobal/cc200/NPY/1035_abide.npy', allow_pickle=True).item()
    timeseries = data["corr"]
    site = data['site']
...
def StratifiedKFold_tr_te(n_splits=5, random_state=42, n_sub=1035):
    if n_sub == 1009:
        data = np.load('D:/dataset/SEED-IV/SEED-IV/cpac/filt_noglobal/cc200/NPY/1009_abide.npy', allow_pickle=True).item()
    else:
        data = np.load('D:/dataset/SEED-IV/SEED-IV_pcp/cpac/filt_noglobal/cc200/NPY/1035_abide.npy', allow_pickle=True).item()
    X = data["corr"]
    site = data['site']
