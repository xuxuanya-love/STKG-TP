...
class GCNConv(gnn.MessagePassing):
    def __init__(self, embed_dim, edge_dim):
        ...
        self.edge_encoder = nn.Linear(edge_dim, embed_dim)   # 边特征编码器
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        deg = utils.degree(row, x.size(0), dtype = x.dtype) + 1    # 计算节点度数
        deg_inv_sqrt = deg.pow(-0.5)  # 度数的倒数的平方根
        deg_inv_sqrt[deg_inv_sqrt == float('inf')] = 0

        norm = deg_inv_sqrt[row] * deg_inv_sqrt[col]        # 计算边的归一化系数,通过将起始节点和结束节点的度数倒数的平方根相乘得到
